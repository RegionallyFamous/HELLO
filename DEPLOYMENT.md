...
The bridge is the only long-running Node process. Customer WordPress sites do not run it and do not configure their own bridge.
...
For plugin-only customer sites, no required setup is needed after activation. The plugin uses the bundled HELLO Bridge endpoint, registers the site automatically, stores a per-site bridge token, and generates its own webhook secret.
...
Use `GET /v1/live` for unauthenticated container liveness checks. Use authenticated `POST /v1/health` for deeper Matrix account checks.
