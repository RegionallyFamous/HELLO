...
const registry = new SiteRegistry(config.siteRegistryPath);
const identityStore = new IdentityStore(config.identityStorePath);
const server = new BridgeServer({
  token: config.bridgeApiToken,
  matrix,
  registry
});
...
console.log(`[hello] bridge listening on :${config.bridgePort}`);

await matrix.start(async (roomId, event) => {
  try {
    await handleMatrixEvent(roomId, event);
  } catch (error) {
    console.error('[hello] bridge event handler failed', error);
  }
});

async function handleMatrixEvent(roomId, event) {
...
async function handleRoomMessage(roomId, event) {
  ...
  const wordpress = new WordPressClient({
    incomingUrl: room.incomingUrl,
    botSecret: room.webhookSecret
  });
...
async function maybeHandleIdentityReply(roomId, userId, message) {
  ...
  if (/^skip$/i.test(normalized)) {
    ...
    await matrix.sendText(roomId, 'Got it. Future messages will use your Matrix display name.');
    return true;
  }
  ...
  await matrix.sendText(roomId, profile.avatarUrl
...
async function sendOnboardingMessage(userId) {
  ...
  if (!roomId) {
    roomId = await matrix.createDirectRoom(userId);
